import { Body, Controller, Delete, Get, Param, Patch, Post, UseGuards, Req } from '@nestjs/common';
import { UsersService } from './users.service';
import { ChangePasswordDto, CreateUserDto, UpdateUserDto, UpdateLocationDto } from './dto';
import { AuthGuard } from '@nestjs/passport';
import { Roles } from '../auth/roles.decorator';
import { RolesGuard } from '../auth/roles.guard';
import { Role } from './user.entity';
import { Request } from 'express';

@Controller('users')
@UseGuards(AuthGuard('jwt'), RolesGuard)
export class UsersController {
  constructor(private service: UsersService) {}

  // Cada quien reporta únicamente su propia ubicación: el id sale del JWT.
  @Patch('update-location')
  updateLocation(@Body() dto: UpdateLocationDto, @Req() req: Request & { user: { userId: string } }) {
    return this.service.updateLocation(req.user.userId, dto.latitude, dto.longitude);
  }

  // Cualquier usuario autenticado cambia SU propia contraseña (el id sale del
  // JWT, nunca de la URL), así el admin principal no depende de nadie más.
  @Patch('me/password')
  changeOwnPassword(@Body() dto: ChangePasswordDto, @Req() req: Request & { user: { userId: string } }) {
    return this.service.changeOwnPassword(req.user.userId, dto.currentPassword, dto.newPassword);
  }

  @Get()
  @Roles(Role.ADMIN)
  findAll() { return this.service.findAll(); }

  @Get('with-location')
  @Roles(Role.ADMIN)
  findAllWithLocation() { return this.service.findAllWithLocation(); }

  @Get(':id')
  @Roles(Role.ADMIN)
  findOne(@Param('id') id: string) { return this.service.findOne(id); }

  @Post()
  @Roles(Role.ADMIN)
  create(@Body() dto: CreateUserDto) { return this.service.create(dto); }

  // El id del admin que ejecuta la acción viaja al servicio para impedir que se
  // degrade, se bloquee o se borre a sí mismo.
  @Patch(':id')
  @Roles(Role.ADMIN)
  update(
    @Param('id') id: string,
    @Body() dto: UpdateUserDto,
    @Req() req: Request & { user: { userId: string } },
  ) {
    return this.service.update(id, dto, req.user.userId);
  }

  @Delete(':id')
  @Roles(Role.ADMIN)
  remove(@Param('id') id: string, @Req() req: Request & { user: { userId: string } }) {
    return this.service.remove(id, req.user.userId);
  }
}
